function EFFECT:Init(data)
    ...
    if !wpn.GetBuff_Override then return end
    ...
    if gmmuzzle then
        ...
        if muzzle then
            if !istable(muzzle) then muzzle = {muzzle} end

            for k, v in pairs(muzzle) do
                ParticleEffectAttach(v, PATTACH_POINT_FOLLOW, mdl, att)
            end
        end
    end
