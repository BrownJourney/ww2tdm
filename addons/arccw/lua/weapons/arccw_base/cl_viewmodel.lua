...
function SWEP:Step_Process(EyePos, EyeAng, velocity)
    ...
    if self:GetState() == ArcCW.STATE_SPRINT and self:SelectAnimation("idle_sprint") and !self.TacSprint then
        velocity = 0
    else
...
function SWEP:GetViewModelPosition(pos, ang)
    ...
    if self:InBipod() then
        ...
        local bang = self:GetBuff_Override("Override_InBipodAng", self.InBipodAng)
        target.pos = asight and asight.Pos or target.pos
        target.ang = asight and asight.Ang or target.ang
        if self.ZeroBipodAngle then
            target.ang = Angle()
        end
    ...
    do
        ...
        if self.ROBroken then
            spos, sang = self:GetBuff("ActivePos", true), self:GetBuff("ActiveAng", true)
        end
        ...
        if self:CanTacSprint() then
            if self:GetState() == ArcCW.STATE_SPRINT and !self:GetReloading() and self:GetShotgunReloading() == 0 and !self:GetNeedCycle() then
                sd = sd * 1.8
                self.TacSprint = true
                aaaapos = aaaapos + Vector(5, -20, -10)
                aaaaang = aaaaang + Angle(90, 0, 0)
            else
                self.TacSprint = false
            end
        end 
