...
function SWEP:SelectAnimation(anim)
    ...
    if self:Clip1() == 0 then
        if self.LastClipCount == 1 and self.Animations[anim .. "_last"] then
            anim = anim .. "_last"
        elseif self.LastClipCount == 0 and self.Animations[anim .. "_empty"] then
            anim = anim .. "_empty"
        end
    end
...
function SWEP:PlayAnimation(key, mult, pred, startfrom, tt, skipholster, ignorereload, absolute, interrupt)
    mult = mult or 1
    pred = pred or false
    ...
    if isnumber(anim.ShellEjectAt) and CLIENT then
...
function SWEP:PlayIdleAnimation(pred)
    ...
    if self:GetState() == ArcCW.STATE_SPRINT and !s and !self:CanTacSprint() then
    ...
    if (self.Sighted or self:GetState() == ArcCW.STATE_SIGHTS) then
        ianim = self:SelectAnimation("idle_sight") or self:SelectAnimation("idle_sights") or self:SelectAnimation("idle_iron") or ianim
    end
    ...
    local clipInfo = self:Clip1()
    if self:GetMagUpCount() > 0 then
        clipInfo = self:GetMagUpCount()
    end

    -- (key, mult, pred, startfrom, tt, skipholster, ignorereload)
    if self:GetInUBGL() and self.Animations.idle_ubgl_empty and self:Clip2() <= 0 then
        ianim = "idle_ubgl_empty"
    elseif self:GetInUBGL() and self.Animations.idle_ubgl then
        ianim = "idle_ubgl"
    elseif (clipInfo == 0 or self:GetNeedCycle()) and self.Animations.idle_empty then
    ...
    self:PlayAnimation(ianim, 1, pred, nil, nil, nil, true, true)
end

function SWEP:GetAnimKeyTime(key, min, rtime)
