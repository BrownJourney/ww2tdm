function SWEP:CanPrimaryAttack()
    ...
    if gamestate and gamestate.Get() != GS_ROUND_PLAYING and gamestate.Get() != GS_WAITING_PLAYERS then return end
    ...
    -- Attempting a bash
    if self:GetState() != ArcCW.STATE_SIGHTS and self.PrimaryBash then self:Bash() return end
...
--
    ...
    if CVAR_ARCADE then
        spread = spread * CFG.ArcadeAccuracyModifier
    end
    ...
    owner:DoCustomAnimEvent(PLAYERANIMEVENT_ATTACK_PRIMARY, 3002)
...
function SWEP:DoPrimaryAnim()
    ...
    if inbipod and iron and self.BrokenIronFire then return end
...
function SWEP:DoRecoil()
    ...
    if CVAR_ARCADE then
        rmul = rmul * CFG.ArcadeRecoilModifier
    end
    ...
    if self:CanBipod(true) and !self.BipodRecoil then
        local b = 0.75
