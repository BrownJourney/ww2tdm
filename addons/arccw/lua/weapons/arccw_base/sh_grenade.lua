...
function SWEP:Throw(stick)
    ...
    self.Owner:DoCustomAnimEvent(PLAYERANIMEVENT_ATTACK_PRIMARY, 3002)

    if stick then force = 0 end
    ...
    self:SetTimer(0.25, function()
        ...
        if stick then
            ...
            if IsValid(tr.Entity) then
                if SERVER then
                    notify.Add(self.Owner, "notify_bomb", "nil", 3)
                end
            end

            local hitpos = tr.HitPos

            if hitpos:Distance(self.Owner:GetPos()) > 100 then
                return
            end

            rocket:SetPos(hitpos)
