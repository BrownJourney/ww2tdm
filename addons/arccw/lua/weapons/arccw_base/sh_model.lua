...
function SWEP:SetupModel(wm)
    ...
    if !wm then
        ...
        vm.RenderOverride = function(v)
            ...
            for i, k in pairs(self:GetBuff_Override("Override_CaseBGs", self.CaseBGs) or {}) do
                ...
                local allowed = true
                if wep.CaseBGs_OnlyReload then
                    allowed = wep:GetReloading()
                end
                
                if self:GetVisualClip() == i and allowed then
                    vm:SetBodygroup(k.ind, k.bg)
                    break
                else
                    if !self:GetBodyGroups()[k.ind + 1] then continue end
                    local maxValue = #self:GetBodyGroups()[k.ind + 1]['submodels']
                    vm:SetBodygroup(k.ind, maxValue)
                end
            end
            ...
            for i, k in pairs(self:GetBuff_Override("Override_BulletBGs", self.BulletBGs) or {}) do
                ...
                if self:GetVisualBullets() == i then
                    ...
                    local data = self:GetBodyGroups()[k.ind + 1]
                    if !data then continue end
                    local maxValue = #data['submodels']
                    if self.InversedBulletBGs then
                        maxValue = 0
                    end
                    vm:SetBodygroup(k.ind, maxValue)
                end
    ...
    for i, k in pairs(self.Attachments) do
        ...
        if wm and atttbl.HideWorldModel then continue end

        local model = ClientsideModel(wm and atttbl.WorldModel or atttbl.Model)
        ...
        if atttbl.BoneMerge then
            ...
            model:SetParent(self)
            local vpos = k.Offset.vvpos || k.Offset.vpos || vector_origin
            local vang = k.Offset.vvang || k.Offset.vang || vector_origin
            model:SetPos(model:GetPos() + Vector(10, 0, -10))
            model:SetAngles(model:GetAngles() + Angle(0, 0, 90))
        end
        ...
        if atttbl.HolosightPiece then
            ...
            if k.BoneMerge or atttbl.HSPBonemerge then
                local parent = self:GetOwner():GetViewModel()
                ...
                local csent = (k.VElement or {}).Model
                local bmParent = IsValid(csent) and csent or parent

                hspmodel:SetParent(bmParent) -- it actually parents hsp to scope and not to the model
                hspmodel:SetPos(hspmodel:GetPos() + (atttbl.HolosightOffset or vector_origin))
                hspmodel:SetModelScale(atttbl.HolosightMScale or 1)
            else
                hspmodel:SetParent(self)
            end
            ...
            hspelement.ModelOffset = atttbl.HolosightOffset or atttbl.ModelOffset
            hspelement.OffsetAng = element.OffsetAng
...
function SWEP:GetFromReference(boneid)
    ...
    ArcCW.ReferenceModel:SetRenderMode(RENDERMODE_TRANSCOLOR)
    ArcCW.ReferenceModel:SetColor(Color(255, 255, 255, 0))
