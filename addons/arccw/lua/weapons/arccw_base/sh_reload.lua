...
function SWEP:Reload()
    ...
    local bulletsShot = load - self:Clip1()
    local useSgReload = false
    if self.StripperHybrid and bulletsShot < (self.StripperSize or 5) then
        if self.Animations["sgreload_start"] then
            useSgReload = true
        else
            return
        end
    ...
    local mult = self:GetBuff_Mult("Mult_ReloadTime")
    if self.Owner:GetTrait() == TRAIT_QUICKRELOAD and self:Clip1() == 0 then
        mult = 0.77
    end

    if shouldshotgunreload or useSgReload then
        ...
        local reloadtime2 = self:GetAnimKeyTime(anim, false, true) * mult
        ...
        if reloadtime2 > 0 then
            self:SetInternalReload(CurTime() + reloadtime2)
        end
    end
    ...
    self.Owner:DoCustomAnimEvent(PLAYERANIMEVENT_RELOAD, 0)

    self:GetBuff_Hook("Hook_PostReload")
end

function SWEP:ReloadTimed(onlyMag)
...
function SWEP:RestoreAmmo(count)
    ...
    if self.ForceDefaultClip and (self:Clip1() > 0 or !self.EmptyFull) then
        count = math.min(count, self.ForceDefaultClip)
    end
...
function SWEP:SelectReloadAnimation()
    ...
    if self:InBipod() then
        if self.Animations.reload_empty and self:Clip1() == 0 then
            ret = "reload_bipod_empty"
        else
            ret = "reload_bipod"
        end
    end

    //if self:InBipod() and self.Animations[ret .. "_bipod"] then
    //    ret = ret .. "_bipod"
    //end
