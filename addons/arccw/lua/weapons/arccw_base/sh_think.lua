...
function SWEP:Think()
    ...
    if self:GetGrenadePrimed() and self.Sticky and owner:KeyDown(IN_ATTACK2) and (!game.SinglePlayer() or SERVER) then
        self:Throw(true)
    end
    ...
    if self:GetInternalReload() != 0 and CurTime() > self:GetInternalReload() then
        self:ReloadTimed(true)
        self:SetInternalReload(0)
    end
    ...
    if !self:GetReloading() and self:GetShotgunReloading() == 0 and !self:GetNeedCycle() then
        local lastAnim = self.lastAnim or ""
        if !lastAnim:find("cycle") or self:GetNextIdle() <= CurTime() then
            if self:GetState() == ArcCW.STATE_SPRINT or self:GetState() == ArcCW.STATE_IDLE or self:GetState() == ArcCW.STATE_SIGHTS then
                if self.lastState and self.lastState != self:GetState() then
