...
IGS("Премиум-статус", "premium_status"):SetULXGroup("premium")
	:SetPrice(350)
	:SetDiscountedFrom(400)
	:SetTerm(30)
	:SetCategory("Привилегии")
	:SetDescription([[
	Премиум-статус содержит в себе следующие плюшки:
		
	- Повышенные награды за выполнение заданий
	- Возможность смены параметров персонажа во вкладке настроек
	- Специальное выделение в табе
	- Возможность зайти за команду вне зависимости от лимита
	- Автобаланс команд не касается вас
	- Иммунитет к вотекику (голосованию за исключение)
	- Иммунитет к АФК-кику
	- Возможность наблюдать наблюдать за игрой
	- Возможность смены обликов оружия (В СКОРОМ ВРЕМЕНИ)
	]])


local BOOST_DESC = [[
	Жетон удвоенной прокачки, который активируется СРАЗУ ПОСЛЕ ПОКУПКИ.
	Удваивает весь получаемый опыт (общий и классовый).
	Не влияет на награды за выполнение заданий.
	Если у вас уже есть активный бонус удвоенной прокачки, то данный жетон увеличит активное время на указанную длительность.
	]]

local function addBoost(ply, hours)
	ply:AddBoost(hours)
end

IGS("Двойной буст опыта на 1 час", "xpboost_1hour")
	:SetPrice(100)
	:SetStackable()
	:SetOnActivate(function(ply)
		addBoost(ply, 1)
	end)
	:SetDescription(BOOST_DESC)

IGS("Двойной буст опыта на 2 часа", "xpboost_2hour")
	:SetPrice(150)
	:SetStackable()
	:SetOnActivate(function(ply)
		addBoost(ply, 2)
...
IGS("Двойной буст опыта на 6 часов", "xpboost_6hour")
	:SetPrice(300)
	:SetStackable()
	:SetOnActivate(function(ply)
		addBoost(ply, 6)
...
IGS("Двойной буст опыта на 12 часов", "xpboost_12hour")
	:SetPrice(400)
	:SetStackable()
	:SetOnActivate(function(ply)
		addBoost(ply, 12)
...
IGS("Перманентный буст опыта", "xpboost_perm")
	:SetPrice(1200)
	:SetOnActivate(function(ply)
		addBoost(ply, -1)
	end)
...
IGS("Пакет воителя", "faction_unlock")
	:SetPrice(3000)
	:SetDiscountedFrom(4000)
	:SetCategory("Пакеты")
	:SetOnActivate(function(ply)
		ply:UnlockEverything()
	end)
	:SetDescription([[
	Данный пакет открывает доступ ко всем классам и снаряжению во всех фракциях (даже в тех, которые будут добавлены в будущем).
	Покупая данный пакет, вы можете не беспокоиться о прокачке, ведь у вас все будет открыто. Просто наслаждайтесь игрой с широким арсеналом оружия разных фронтов. 
	]])

local function giveCoins(ply, amount)
	ply:AddCoins(amount)
end

IGS("Монеты снабжения (4)", "4_coins")
	:SetPrice(80)
	:SetCategory("Монеты")
	:SetStackable()
	:SetOnActivate(function(ply)
	...
	:SetDescription([[
	Вы получите 4 монет снабжения, за которые можно разблокировать классы и снаряжения внутри классов.
	]])

IGS("Монеты снабжения (10)", "10_coins")
	:SetPrice(150)
	:SetCategory("Монеты")
	:SetStackable()
	:SetOnActivate(function(ply)
	...
	:SetDescription([[
	Вы получите 10 монет снабжения, за которые можно разблокировать классы и снаряжения внутри классов.
	]])

IGS("Монеты снабжения (20)", "20_coins")
	:SetPrice(250)
	:SetCategory("Монеты")
	:SetStackable()
	:SetOnActivate(function(ply)
	...
	:SetDescription([[
	Вы получите 20 монет снабжения, за которые можно разблокировать классы и снаряжения внутри классов.
	]])

IGS("Монеты снабжения (50)", "50_coins")
	:SetPrice(450)
	:SetCategory("Монеты")
	:SetStackable()
	:SetOnActivate(function(ply)
	...
	:SetDescription([[
	Вы получите 50 монет снабжения, за которые можно разблокировать классы и снаряжения внутри классов.
	]])

IGS("Монеты снабжения (100)", "100_coins")
	:SetPrice(700)
	...
	:SetDescription([[
	Вы получите 100 монет снабжения, за которые можно разблокировать классы и снаряжения внутри классов.
	]])
