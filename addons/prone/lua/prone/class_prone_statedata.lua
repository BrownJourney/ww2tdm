...
function prone.PlayerStateData:UpdateDataOnProneEnter()
	...
	local seq = PRONE_ANIMS["down"]

	self.EnterProneAnimLength = self.Player:SequenceDuration(self.Player:LookupSequence(seq)) * sequenceMult
...
function prone.PlayerStateData:UpdateOnProneEnd(ply)
	...
	local seq = PRONE_ANIMS["standup"]

	self.EndProneAnimLength = self.Player:SequenceDuration(self.Player:LookupSequence(seq)) * sequenceMult
