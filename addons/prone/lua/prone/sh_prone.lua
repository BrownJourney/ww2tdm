...
function prone.Enter(ply)
	...
	ply:DoCustomAnimEvent(PLAYERANIMEVENT_CUSTOM, 211)
...
function prone.End(ply)
	...
	-- Play the animation.
	ply:DoCustomAnimEvent(PLAYERANIMEVENT_CUSTOM, 212)
...
hook.Add("SetupMove", "prone.Handle", function(ply, cmd, cuc)
	local proneMoveSpeed = CFG and CFG.WalkSpeed / 3.5 or 100
	...
	if ply:IsProne() then
		...
		if ply:GetProneAnimationLength() >= CurTime() then
			...
			local moveSpeed = proneMoveSpeed
	...
	if cuc:GetImpulse() == PRONE_IMPULSE then
		prone.Handle(ply)
	end
end)
...
timer.Create("prone.Manage", 0.5, 0, function()
	for i, v in ipairs(player.GetAll()) do
		local state = v:GetProneAnimationState()
		if GetBehaviourActivity[state] then
			GetBehaviourActivity[state](v)
		end
