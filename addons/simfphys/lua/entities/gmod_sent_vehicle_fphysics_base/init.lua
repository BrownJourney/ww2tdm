...
function ENT:Think()
	...
	if self.NextTick < Time then
		...
		if self:WaterLevel() > 2 and (self.NextDamage or 0) < CurTime() then
			self.NextDamage = CurTime() + 1
			self:ApplyDamage(150, DMG_BULLET)
		end
...
function ENT:Use( ply )
	...
	if gamestate.Get() != GS_ROUND_PLAYING and gamestate.Get() != GS_WAITING_PLAYERS then return end
...
function ENT:IsReallyEmpty()
	local mainDriver = self:GetDriver()

	if IsValid(mainDriver) then
		return false
	end

	for k, v in pairs(self:GetPassengerSeats()) do
		local sDriver = v:GetDriver()

		if IsValid(sDriver) then
			return false
		end
	end
...
function ENT:SetPassenger( ply )
	...
	if self.team and self.team != ply:Team() then return end

	if !self.tmrInitialized then
		self.tmrInitialized = true
		local timerid = "RemoveIfNotUsed_"..self:GetCreationID()
		timer.Create(timerid, 60, 0, function()
			if !IsValid(self) then
				return
			end
			if self:GetVelocity():Length() == 0 and self:IsReallyEmpty() then
				self.notUsed = (self.notUsed or 0) + 1

				if self.notUsed >= 2 then
