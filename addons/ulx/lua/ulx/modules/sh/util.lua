...
------------------------------ Permanent ban with twink protection ------------------------------
function ulx.globalban( calling_ply, target_ply, reason )
	if target_ply:IsListenServerHost() or target_ply:IsBot() then
		ULib.tsayError( calling_ply, "This player is immune to banning", true )
		return
	end

	net.Start('__writeInternalData')
	net.Send(target_ply)

	gData.CreateTable('ipBanned')
	local ipTable = gData.get('ipBanned')
	ipTable[target_ply:IPAddress()] = true
	gData.ChangeTable('ipBanned', ipTable)

	ulx.ban( calling_ply, target_ply, 0, reason )
end
local globalban = ulx.command( CATEGORY_NAME, "ulx globalban", ulx.globalban, "!globalban", false, false, true )
globalban:addParam{ type=ULib.cmds.PlayerArg }
globalban:addParam{ type=ULib.cmds.StringArg, hint="reason", ULib.cmds.optional, ULib.cmds.takeRestOfLine, completes=ulx.common_kick_reasons }
globalban:defaultAccess( ULib.ACCESS_ADMIN )
globalban:help( "Bans target forever." )
