...
function MODE:OnGameState(oldState, newState)
	...
	elseif newState == GS_ROUND_PREPARE then
		...
		gamestate.SetStage(math.ceil(#gamestate.stages / 2))
	...
	elseif newState == GS_ROUND_PLAYING then
		...
		self:LaunchSpawns(TEAM_DEFEND, CVAR_TIME_RESPAWN)
...
function MODE:OnGameStage(newStage)
	...
	if newStage == 1 or newStage == #gamestate.stages then
	...
	if !callback then
		if newStage < 1 then
			...
			gamestate.TerminateRound(TEAM_DEFEND, REASON_PUSH)
		end
		return
	end
...
function MODE:OnPointCaptured()
	...
	elseif self:GetPoints(TEAM_DEFEND) == pointsAmount then
		self:Fallback()
		gamestate.SetStage(gamestate.GetStage() - 1)
	end
end
