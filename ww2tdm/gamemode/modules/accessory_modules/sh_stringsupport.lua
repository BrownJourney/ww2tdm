...
function utf8upper (s)
	return string.upper(utf8replace(s, utf8_lc_uc))
end
-- identical to string.lower except it knows about unicode simple case conversions
function utf8lower (s)
	return string.lower(utf8replace(s, utf8_uc_lc))
end

function utf8latine (s)
	return utf8replace(s, utf8_en_lc)
end
...
function utf8gensub(str, sub_len)
...
function classMatchGenerator(class, plain)
	local codes = {}
	local ranges = {}
	local ignore = false
	local range = false
...
function utf8subWithBytes (s, i, j, sb)
	-- argument defaults
	j = j or -1
	local pos = sb or 1
	local bytes = len(s)
...
function matcherGenerator(regex, plain)
	local matcher = {
		functions = {},
		captures = {}
	}
...
-- string.gmatch
function utf8gmatch(str, regex, all)
...
-- string.gsub
function utf8gsub(str, regex, repl, limit)
	limit = limit or -1
	local ret = ''
	local prevEnd = 1
